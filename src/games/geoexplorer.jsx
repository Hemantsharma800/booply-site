...
import { getMockAiResponse } from '../data/geoAiData.js';
...
function Earth({ onScan }) {
    ...
    return (
        <mesh ref={earthRef} onClick={onScan} scale={2.4}>
...
function GeoExplorer({ onExit }) {
    ...
    const handleGlobeClick = () => {
        setIsScanning(true);
        setData(null);
        // Simulate AI Coordinate Analysis
        setTimeout(() => {
            setData(getMockAiResponse());
            setIsScanning(false);
        }, 1800);
    };
    ...
    return (
        <div className="geo-ai-arena">
            <Canvas camera={{ position: [0, 0, 6] }}>
                ...
                <Suspense fallback={<Html>Loading Satellite...</Html>}>
            ...
            <div className="geo-overlay">
                ...
                {isScanning && <div className="scanning-bar">SCANNING EARTH SURFACE...</div>}
                ...
                {data && !isScanning && (
                    <div className="info-panel slide-in">
                        ...
                        <div className="cities-row">
                            {data.cities.map(c => <span key={c} className="city-pill">{c}</span>)}
                        </div>
